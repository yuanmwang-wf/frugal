...
// TODO copied from go library, as we get a better idea of the API for this
// we should deduplicate what we can
type (
	...
	//
	// TODO I don't want the frugal args here as it confuses the value of the
	// middleware people can put on the client they provide. IMO this should
	// just be an FContext and the http headers, maybe with a binary json blob
	// we receive if people will need that for some weird reason
	InvocationHandler func(service reflect.Value, method reflect.Method, headers http.Header, args Arguments) Results
	...
	// service call.
	GatewayMiddleware func(InvocationHandler) InvocationHandler
...
func (a Arguments) Context() frugal.FContext {
	return a[0].(frugal.FContext)
}

// Context sets the given FContext as the first argument.
func (a Arguments) SetContext(ctx frugal.FContext) {
	a[0] = ctx
}
...
// generated code.
func (m *Method) Invoke(headers http.Header, args Arguments) Results {
	return m.handler(m.proxiedStruct, m.proxiedMethod, headers, args)
}

// AddMiddleware wraps the Method with the given ServiceMiddleware. This should
// only be called by generated code.
func (m *Method) AddMiddleware(middleware GatewayMiddleware) {
...
// only be called by generated code.
func NewMethod(proxiedHandler, method interface{}, methodName string, middleware []GatewayMiddleware) *Method {
	var (
...
func composeMiddleware(method reflect.Value, middleware []GatewayMiddleware) InvocationHandler {
...
func newInvocationHandler(method reflect.Value) InvocationHandler {
	return func(_ reflect.Value, _ reflect.Method, _ http.Header, args Arguments) Results {
